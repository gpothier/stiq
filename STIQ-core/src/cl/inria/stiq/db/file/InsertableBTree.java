...
//			if (LOG) System.out.println(internalPageToString(thePage));
	...
	protected PageIOStream insertLeafKey(long aKey, boolean aAllowOverwrite)
	{
		...
		int[] theIndexes = new int[DBConfig.DB_MAX_INDEX_LEVELS]; // MAX instead of root level in hope the compiler can optimize better.
		Page[] thePages = new Page[DBConfig.DB_MAX_INDEX_LEVELS];
