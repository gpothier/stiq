...
*/
package cl.inria.stiq.db.file;

import java.io.File;
import java.io.FileNotFoundException;
import java.io.PrintWriter;

import cl.inria.stiq.db.DBConfig;
import cl.inria.stiq.db.DebugFlags;
import cl.inria.stiq.db.Stats.Account;
import cl.inria.stiq.db.file.Page.ChainedPageIOStream;
import cl.inria.stiq.db.file.Page.PageIOStream;
import cl.inria.stiq.db.file.TupleFinder.Match;
...
public abstract class StaticBTree<T extends Tuple>
{
	 ...
	 */
	private MyChainedPageIOStream<T>[] itsChains = new MyChainedPageIOStream[DBConfig.DB_MAX_INDEX_LEVELS];
	
	/**
	 * Last written key for each level.
	 */
	private long[] itsLastKeys = new long[DBConfig.DB_MAX_INDEX_LEVELS];
	
	/**
	 * The keys currently being written for each level.
	 */
	private long[] itsCurrentKeys = new long[DBConfig.DB_MAX_INDEX_LEVELS];

	/**
	 * Number of tuples before the beginning of the current page for each level.
	 */
	private long[] itsTupleCount = new long[DBConfig.DB_MAX_INDEX_LEVELS];
...
//	private int[] itsPagesCount = new int[DB_MAX_INDEX_LEVELS];
	...
	public static int getSerializedSize()
	{
		...
		theResult += DBConfig.DB_MAX_INDEX_LEVELS * (
